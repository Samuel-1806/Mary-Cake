...
(() => {
  ...
  function buildSmartphone(textureFile) {
    ...
    // Material físico premium: reduzimos metalness/aumentamos roughness
    // em relação à versão anterior — sem um envMap real na cena, um
    // metalness muito alto (0.9) com pouca rugosidade lê como plástico
    // brilhante genérico (o efeito "celular antigo" relatado) em vez de
    // alumínio anodizado fosco, que é o acabamento real dos mockups de
    // referência.
    ...
    // Antes era um círculo único, liso — "seco" demais pra ler como
    // iPhone de verdade. Agora é uma placa quadrada de cantos
    // arredondados com 3 lentes em arranjo diagonal (mesmo desenho
    // clássico das câmeras triplas, tipo iPhone 11 Pro em diante) mais
    // um flash pequeno no canto livre.
    ...
    // usada pra tela, só espelhada) — não em -bodyDepth/2. A posição
    // antiga (-bodyDepth/2 - 0.001 = -0.081) ficava ~0.009 unidades
    // DENTRO do corpo (a casca de trás real está em -0.09), quase colada
    // à malha do chassi. Com várias camadas empilhadas a 0.001–0.002 de
    // distância uma da outra ali dentro, a precisão do depth buffer
    // (câmera com near/far = 0.1/100, uma proporção de 1000:1) "sorteava"
    // a cada frame quem vencia o teste de profundidade — daí o
    // aparece/some. Agora o módulo nasce claramente FORA da casca, com
    // folgas maiores entre as camadas.
    const bodyBackZ = -bodyFrontZ;

    const camPlateSize = 0.42;
    const camPlateRadius = 0.11;
    ...
    // Cada lente = anel metálico (bezel) + vidro escuro glossy por cima,
    // à frente da placa pra dar sensação de relevo/profundidade — com
    // folga bem maior que antes (0.012/0.008 em vez de 0.001/0.002) e
    // polygonOffset em cada camada, pra nunca mais brigar com a placa
    // nem com o corpo pelo mesmo pixel de profundidade.
    function buildLens(radius) {
      const lensGroup = new THREE.Group();
      const ringGeo = new THREE.CircleGeometry(radius, 24);
      const ringMat = new THREE.MeshStandardMaterial({
        color: 0x2a2a2d, metalness: 0.8, roughness: 0.25, transparent: true, opacity: 1,
        polygonOffset: true, polygonOffsetFactor: -2, polygonOffsetUnits: -2
      });
      lensGroup.add(new THREE.Mesh(ringGeo, ringMat));

      const glassGeo = new THREE.CircleGeometry(radius * 0.7, 24);
      const glassMat = new THREE.MeshStandardMaterial({
        color: 0x040405, metalness: 0.9, roughness: 0.08, transparent: true, opacity: 1,
        polygonOffset: true, polygonOffsetFactor: -3, polygonOffsetUnits: -3
      });
      const lensGlass = new THREE.Mesh(glassGeo, glassMat);
      lensGlass.position.z = 0.008;
      lensGroup.add(lensGlass);

      return lensGroup;
    }

    const lensRadius = 0.09;
    const lensOffset = 0.1;
    [[-lensOffset, lensOffset], [lensOffset, lensOffset], [-lensOffset, -lensOffset]].forEach(([lx, ly]) => {
      const lens = buildLens(lensRadius);
      lens.position.set(lx, ly, 0.012);
      camModule.add(lens);
    });

    // Flash LED no canto oposto às lentes
    const flashGeo = new THREE.CircleGeometry(0.024, 16);
    const flashMat = new THREE.MeshStandardMaterial({
      color: 0xd8cba3, metalness: 0.2, roughness: 0.5, transparent: true, opacity: 1,
      polygonOffset: true, polygonOffsetFactor: -2, polygonOffsetUnits: -2
    });
    const flash = new THREE.Mesh(flashGeo, flashMat);
    flash.position.set(lensOffset, -lensOffset, 0.012);
    camModule.add(flash);

    camModule.position.set(-bodyWidth / 2 + 0.34, bodyHeight / 2 - 0.46, bodyBackZ - 0.006);
  ...
  tl.to('#hero-logo-solid', { opacity: 0, scale: 0.9, duration: 0.6, ease: 'power3.in' }, 0.3)
    ...
    .to(phoneCenter.rotation, {
      y: '+=' + Math.PI * 1.85, x: '+=0.12',
      duration: 1.8, ease: 'sine.inOut'
    }, 4.0)
    .to(phoneCenter.position, { x: -0.9, y: 0.05, z: 0.9, duration: 0.9, ease: 'sine.inOut' }, 4.0)
    .to(phoneCenter.position, { x: 0.85, y: -0.05, z: 0.4, duration: 0.9, ease: 'sine.inOut' }, 4.9)
    .to(phoneCenter.scale, { x: 1.18, y: 1.18, z: 1.18, duration: 0.9, ease: 'sine.inOut' }, 4.0)
    .to(phoneCenter.scale, { x: 1, y: 1, z: 1, duration: 0.9, ease: 'sine.inOut' }, 4.9)

    // ---- SEÇÃO 2 (Ateliê — ÂNCORA LATERAL + JANELA VIVA): o celular
    // desliza para a lateral direita, cedendo a metade esquerda da tela
    // ao texto, e se FIXA ali — virando de frente para a câmera (em vez
    // de continuar girando) com um leve zoom, como se sua tela virasse
    // uma janela viva para o conteúdo ao lado. Toda a transição usa
    // power2.inOut, dando o mesmo "peso" luxuoso pedido para as trocas
    // de estado (Hero -> Lateral -> Centro -> Footer).
    .to(phoneCenter.position, { x: 2.1, y: -0.1, z: -0.5, duration: 2, ease: 'power2.inOut' }, 6.0)
    .to(phoneCenter.rotation, { x: 0.02, y: 0.18, z: 0, duration: 2, ease: 'power2.inOut' }, 6.0)
    .to(phoneCenter.scale, { x: 1.08, y: 1.08, z: 1.08, duration: 2, ease: 'power2.inOut' }, 6.0)
    ...
    .to(phoneCenter.rotation, { x: 0.02, y: 0.22, z: -0.015, duration: 1.4, ease: 'power2.inOut' }, 8.1)
    .to(phoneCenter.scale, { x: 1.06, y: 1.06, z: 1.06, duration: 1.4, ease: 'power2.inOut' }, 8.1)
