...
(() => {
  ...
  function buildSmartphone(textureFile) {
    ...
    // Reservamos uma faixa de bezel de verdade no TOPO (mesma lógica do
    // mockup DOM .telas-phone-shell, que tem padding + notch antes da
    // tela começar) — antes a tela ia quase de ponta a ponta (bezel de
    // 0.05 nos dois lados) e a pílula da câmera frontal acabava caindo
    // por cima do próprio cabeçalho do app (cobrindo o logo "Mary
    // Cake"). Agora a tela nasce mais abaixo, deixando uma moldura preta
    // reservada no topo — exatamente onde a pílula se encaixa, sem
    // tampar nenhum conteúdo real do print.
    ...
    // --- Câmera frontal / notch (pílula) — mesmo desenho visual dos
    // mockups DOM da seção "Dentro do App" (.telas-phone-notch): uma
    // pílula preta, centralizada, sentada na moldura reservada acima da
    // tela (nunca sobre o conteúdo do print). Usa MeshBasicMaterial
    // (não-iluminado) para nunca "brilhar" sob a luz de estúdio — um
    // notch real nunca reflete, só absorve luz.
    const notchWidth = bodyWidth * 0.34;
    const notchHeight = 0.09;
    ...
    notch.position.set(0, bodyHeight / 2 - topBezel / 2, bodyFrontZ + 0.014);
