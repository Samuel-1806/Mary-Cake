...
(function () {
  ...
  function initPhoneTilt() {
    ...
    if (!chassis || isMobile()) return;

    const MAX_X = 14, MAX_Y = 22;
    let curX = 2, curY = -6, tX = 2, tY = -6;
    let raf;

    function update() {
      curX = lerp(curX, tX, 0.09);
      curY = lerp(curY, tY, 0.09);
      chassis.style.transform = `perspective(1200px) rotateX(${curX}deg) rotateY(${curY}deg)`;
      raf = requestAnimationFrame(update);
    }

    /* Acompanha o cursor por toda a extensão da janela, não apenas
       sobre o mockup, para um efeito mais fluido e amplo. */
    window.addEventListener('mousemove', e => {
