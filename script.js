...
(() => {
  ...
  // ---- SEÇÃO 1 (Hero -> Ateliê): os grânulos do logo se desintegram,
  // flutuam brevemente e são realocados matematicamente para os vértices
  // do chassi do celular central. SÓ DEPOIS dessa remontagem o celular
  // real aparece (cross-fade) e a coreografia de scroll prossegue —
  // nunca um corte abrupto.
  tl.to(granuleMorph, {
      scatter: 1, duration: 0.32, ease: 'sine.inOut', onUpdate: updateGranules
    }, 0.05)
    .to(granuleMorph, {
      scatter: 0, t: 1, duration: 0.58, ease: 'power2.inOut', onUpdate: updateGranules
    }, 0.36)
    // Celular real assume o lugar dos grânulos assim que o morph termina
    .set(phoneCenter, { visible: true }, 0.92)
    .fromTo(phoneCenter.scale,
      { x: 0.001, y: 0.001, z: 0.001 },
      { x: 1, y: 1, z: 1, duration: 0.22, ease: 'back.out(1.6)' }, 0.92)
    .to(granuleMat, { opacity: 0, duration: 0.2, ease: 'power1.in' }, 0.9)
    .set(logoGranules, { visible: false }, 1.05)
