...
(() => {
  ...
  const bgVertexShader = `
    ...
    void main() {
      ...
      // Onda contínua e orgânica — amplitude discreta: presença sutil,
      // uma respiração de fundo, nunca uma ondulação visível que compita
      // com o conteúdo em primeiro plano.
      float wave = sin(pos.x * 0.5 + uTime * 0.35) * 0.09
                 + cos(pos.y * 0.42 - uTime * 0.27) * 0.07;
  ...
  const bgFragmentShader = `
    ...
    void main() {
      ...
      vec2 grid = vUvLocal * 148.0 + vec2(vWave, vBump) * 0.12;
      ...
      float breathe = clamp((vWave + 0.09) / 0.18, 0.0, 1.0);
  ...
  // Bolo de DOIS andares, como pedido — proporções clássicas de vitrine
  // de confeitaria: base generosa, segundo andar nitidamente menor.
  const cakeTiers = [
    { radius: 1.14, height: 0.7, baseY: 0 },
    { radius: 0.76, height: 0.58, baseY: 0.7 }
  ];
  const cakeTopperRadius = 0.12;
  const cakeTopTier = cakeTiers[cakeTiers.length - 1];
  const cakeTopperY = cakeTopTier.baseY + cakeTopTier.height + 0.15;
  ...
  // Paleta de confeitaria: creme quente na base, um rosé muito sutil no
  // segundo andar — eco discreto de --color-rose da marca, sem competir
  // com o dourado dos detalhes.
  const cakeIcingCream = new THREE.Color(0xFAF2E4);
  const cakeIcingRose = new THREE.Color(0xF6E3DC);
  const cakeGold = 0xC9A14A;

  cakeTiers.forEach((tier, i) => {
    const isTop = i === cakeTiers.length - 1;

    // Massa/confeito: fosco, macio, com leve variação de cor por andar —
    // parece glacê de verdade, não plástico.
    const tierMat = new THREE.MeshStandardMaterial({
      color: isTop ? cakeIcingRose : cakeIcingCream,
      metalness: 0.02, roughness: 0.48, transparent: true, opacity: 1
    });
    const tierGeo = new THREE.CylinderGeometry(tier.radius, tier.radius * 1.03, tier.height, 56, 1);
    const tierMesh = new THREE.Mesh(tierGeo, tierMat);
    tierMesh.position.y = tier.baseY + tier.height / 2;
    cakeGroup.add(tierMesh);

    // Filete dourado fino na junção de cada andar — eco direto do dourado
    // da malha viva de fundo e do chassi metálico dos celulares.
    const ringMat = new THREE.MeshStandardMaterial({
      color: cakeGold, metalness: 0.9, roughness: 0.22, transparent: true, opacity: 1
    });
    const ringGeo = new THREE.TorusGeometry(tier.radius, 0.015, 10, 64);
    const ring = new THREE.Mesh(ringGeo, ringMat);
    ring.rotation.x = Math.PI / 2;
    ring.position.y = tier.baseY;
    cakeGroup.add(ring);

    // Cobertura escorrendo (drip): pequenas gotas de glacê na borda
    // superior de cada andar — a assinatura visual de confeitaria
    // autoral pedida no briefing ("bolo desenhado, mas caprichado"),
    // feita só com esferas alongadas para manter o orçamento de
    // geometria leve.
    const dripMat = new THREE.MeshStandardMaterial({
      color: isTop ? cakeIcingRose : cakeIcingCream,
      metalness: 0.02, roughness: 0.5, transparent: true, opacity: 1
    });
    const dripCount = isTop ? 11 : 15;
    const topY = tier.baseY + tier.height;
    for (let d = 0; d < dripCount; d++) {
      const a = (d / dripCount) * Math.PI * 2 + i * 0.35;
      const dripDrop = 0.05 + ((d * 37) % 10) / 10 * 0.08; // pseudo-aleatório, mas determinístico
      const drip = new THREE.Mesh(new THREE.SphereGeometry(0.05, 10, 10), dripMat);
      drip.scale.set(1, 1.4 + dripDrop * 3.2, 1);
      drip.position.set(
        Math.cos(a) * tier.radius * 0.985,
        topY - dripDrop * 0.7,
        Math.sin(a) * tier.radius * 0.985
      );
      cakeGroup.add(drip);
    }
  });

  // Fita rosé na base do primeiro andar — detalhe têxtil que amarra a
  // paleta da marca (--color-rose) ao objeto 3D.
  const ribbonMat = new THREE.MeshStandardMaterial({
    color: 0xC97B86, metalness: 0.12, roughness: 0.42, transparent: true, opacity: 1
  });
  const ribbonGeo = new THREE.CylinderGeometry(
    cakeTiers[0].radius * 1.006, cakeTiers[0].radius * 1.03, 0.1, 56, 1, true
  );
  const ribbon = new THREE.Mesh(ribbonGeo, ribbonMat);
  ribbon.position.y = cakeTiers[0].baseY + 0.18;
  cakeGroup.add(ribbon);

  // Laço da fita: dois laços (esferas achatadas) + nó central, na frente do bolo.
  [-1, 1].forEach((side) => {
    const loop = new THREE.Mesh(new THREE.SphereGeometry(0.095, 16, 16), ribbonMat);
    loop.scale.set(1, 0.6, 0.45);
    loop.position.set(
      side * 0.09,
      cakeTiers[0].baseY + 0.18,
      cakeTiers[0].radius * 0.98
    );
    cakeGroup.add(loop);
  });
  const bowKnot = new THREE.Mesh(new THREE.SphereGeometry(0.055, 14, 14), ribbonMat);
  bowKnot.position.set(0, cakeTiers[0].baseY + 0.18, cakeTiers[0].radius * 1.02);
  cakeGroup.add(bowKnot);

  // Topo: uma pequena rosácea de açúcar (pétalas + centro), desenhada à
  // mão em vez de uma esfera solitária — assinatura de confeitaria de
  // luxo, mesma linguagem dourada dos outros detalhes metálicos.
  const petalMat = new THREE.MeshStandardMaterial({
    color: cakeGold, metalness: 0.85, roughness: 0.2, transparent: true, opacity: 1
  });
  const petalCount = 6;
  for (let p = 0; p < petalCount; p++) {
    const a = (p / petalCount) * Math.PI * 2;
    const petal = new THREE.Mesh(new THREE.SphereGeometry(cakeTopperRadius * 0.6, 14, 14), petalMat);
    petal.scale.set(1, 0.6, 1.5);
    petal.rotation.y = -a;
    petal.position.set(
      Math.cos(a) * cakeTopperRadius * 0.58,
      cakeTopperY,
      Math.sin(a) * cakeTopperRadius * 0.58
    );
    cakeGroup.add(petal);
  }
  const petalCenter = new THREE.Mesh(new THREE.SphereGeometry(cakeTopperRadius * 0.4, 16, 16), petalMat);
  petalCenter.position.y = cakeTopperY;
  cakeGroup.add(petalCenter);
