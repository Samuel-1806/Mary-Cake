...
(() => {
  ...
  /* ---------------------------------------------------------------------
     0c. CURSOR CUSTOMIZADO — círculo fino dourado com lag, expande sobre
     links/botões. Só é inicializado em dispositivos com ponteiro fino
     (mouse/trackpad); em touch, o próprio CSS já esconde o elemento e
     aqui nem chegamos a ligar os listeners de mousemove, evitando custo
     desnecessário em mobile.
     --------------------------------------------------------------------- */
  ...
  if (supportsFinePointer && !isMobile) {
    ...
    if (cursorEl) {
      ...
      // Posição "real" do mouse (sem lag) — usada pelo ponto pequeno.
      let mouseX = window.innerWidth / 2;
      let mouseY = window.innerHeight / 2;

      // Posição "atrasada" do anel — interpolada a cada frame (lerp),
      // criando a sensação de física suave / seda, coerente com o
      // restante da interface.
      let ringX = mouseX;
      let ringY = mouseY;
      const LAG = 0.16; // quanto menor, mais "pesado"/lento o anel
      ...
      window.addEventListener('mousemove', (e) => {
        ...
        mouseY = e.clientY;
        if (!hasMoved) {
          // Evita que o anel "voe" da origem (0,0) até a primeira
          // posição real do mouse — teleporta na primeira leitura.
          ringX = mouseX;
          ringY = mouseY;
          hasMoved = true;
      ...
      function tickCursor() {
        ...
        cursorEl.style.setProperty('--rx', ringX + 'px');
        cursorEl.style.setProperty('--ry', ringY + 'px');
