...
function initFabricBackground() {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      /* ruído 2D leve — usado só para o "grão" fino do tecido */
      float hash(vec2 p){
      ...
      void main(){
        ...
        /* influência do cursor: cai suavemente, sem borda/anel visível */
        float dist = length(auv - mouseUv);
        float influence = uMouseInfluence * smoothstep(0.5, 0.0, dist);
        vec2 dir = (auv - mouseUv) / max(dist, 0.0001);

        /* o tecido se afasta de leve perto do cursor — deformação, não cor */
        vec2 warp = auv + dir * influence * 0.03;

        /* vento: 2-3 ondas bem lentas e largas, quase imperceptíveis */
        float wind = sin(warp.x * 1.1 + uTime * 0.10) * 0.5
                   + sin(warp.y * 0.85 - uTime * 0.075) * 0.5
                   + sin((warp.x + warp.y) * 1.4 + uTime * 0.055) * 0.25;

        /* grão fino do tecido — trama quase invisível, dá textura sem chamar atenção */
        float weaveA = noise(vec2(warp.x * 70.0, warp.y * 5.0));
        float weaveB = noise(vec2(warp.x * 5.0, warp.y * 70.0));
        float weave = (weaveA + weaveB - 1.0) * 0.5;

        float sheen = wind * 0.028 + weave * 0.02;

        /* creme muito próximo do fundo original — variação mínima */
        vec3 creamLight = vec3(0.972, 0.965, 0.953);
        vec3 creamDark  = vec3(0.953, 0.941, 0.918);
        vec3 base = mix(creamDark, creamLight, 0.5 + sheen);

        /* aquecimento sutil só bem perto do cursor — sem mancha saturada */
        vec3 warmTouch = vec3(0.886, 0.784, 0.706);
        float warmAmount = influence * 0.22;
        vec3 color = mix(base, warmTouch, warmAmount);
