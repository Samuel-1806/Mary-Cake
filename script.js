...
(() => {
  ...
  const bgVertexShader = `
    ...
    float clothWave(vec2 p, float t) {
      return sin(p.x * 0.5 + t * 0.45) * 0.40
           + cos(p.y * 0.42 - t * 0.36) * 0.32
           + sin((p.x + p.y) * 0.3 + t * 0.58) * 0.22
           + sin(p.x * 0.15 - p.y * 0.21 + t * 0.16) * 0.26; // dobra larga e lenta (drapeado)
    }
    ...
    void main() {
      ...
      vBump = clamp(clothBump(p, uMouse, uMouseActive) / 1.35, 0.0, 1.0);
  ...
  const bgFragmentShader = `
    ...
    void main() {
      ...
      // Tons derivados SOMENTE das duas cores da paleta — mistura mais
      // generosa que antes para que o relevo do tecido seja LIDO à
      // primeira vista, não apenas em teoria: vale da dobra mais quente
      // (mais dourado) e crista mais luminosa (dourado clareado).
      ...
      clothColor = mix(clothColor, uGoldColor, sheen * 0.24 + sheen2 * 0.12);
      clothColor += highlightTone * specular * 0.7;

      float bumpStrength = clamp(vBump, 0.0, 1.0);
      clothColor = mix(clothColor, highlightTone, bumpStrength * 0.6);
      ...
      float baseAlpha = 0.30 + (1.0 - diffuse) * 0.22;
      float sheenAlpha = sheen * 0.16 + sheen2 * 0.08;
      float specAlpha = specular * 0.45;
      float bumpAlpha = bumpStrength * 0.5;

      float alpha = clamp(baseAlpha + sheenAlpha + specAlpha + bumpAlpha, 0.0, 0.85);
