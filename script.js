...
(() => {
  ...
  const PHONE_CORNER_RADIUS = 0.17;
  ...
  function buildSmartphone(textureFile) {
    ...
    // Bezel fino e UNIFORME nos 4 lados — mesma proporção do mockup DOM
    // .telas-phone-shell (padding: 0.5rem em volta de todo o aparelho,
    // sem faixa preta reservada no topo). O visual "celular antigo"
    // reportado vinha exatamente dessa faixa preta grossa (0.32) que só
    // existia no topo: nenhum iPhone atual tem essa moldura — a ilha/
    // notch flutua SOBRE a tela, não empurra o conteúdo pra baixo.
    const screenWidth = bodyWidth - 0.09;
    const topBezel = 0.085;
    const bottomBezel = 0.085;
    ...
    // para fora do canto curvo do corpo perto das bordas, o que também
    // contribuía para a leitura de aparelho antigo/genérico.
    const screenCornerRadius = Math.max(cornerRadius - 0.05, 0.06);
    ...
    // --- Câmera frontal / "ilha dinâmica" (pílula) — pequena, estreita e
    // FLUTUANDO sobre a própria tela (igual a um iPhone 14/15 real e igual
    // ao .telas-phone-notch do mockup DOM: este também fica por cima da
    // tela, não numa faixa preta reservada acima dela). Antes a pílula era
    // larga (34% da largura) e vivia numa moldura preta grossa só no topo —
    // isso é o que lia como "celular antigo". Agora é uma pílula pequena
    // (perto de 24% da largura), encostada perto da borda superior da
    // tela, sobrepondo o conteúdo por uma fatia mínima — exatamente como
    // a ilha dinâmica real. Usa MeshBasicMaterial (não-iluminado) para
    // nunca "brilhar" sob a luz de estúdio — um notch real nunca reflete,
    // só absorve luz.
    const notchWidth = bodyWidth * 0.24;
    const notchHeight = 0.075;
    const notchShape = createRoundedRectShape(notchWidth, notchHeight, notchHeight / 2);
    ...
    notch.position.set(0, screenTopEdge - notchHeight * 0.85, bodyFrontZ + 0.016);
