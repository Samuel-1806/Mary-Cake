...
(() => {
  ...
  /* ---------------------------------------------------------------------
     0a. LENIS SMOOTH SCROLL & WEB AUDIO API ENGINE (ÁUDIO TÁTIL)
     --------------------------------------------------------------------- */
  let lenis = null;
  ...
  // Web Audio API engine de sintetização cristalina para microinterações
  const AudioEngine = {
    ctx: null,
    muted: true,
    init() {
    ...
    playChime(freq = 880, type = 'sine', duration = 0.4, vol = 0.05) {
    ...
    playTick() {
      this.playChime(1200, 'sine', 0.06, 0.02);
    },
    playAtmosphereChime() {
      this.playChime(523.25, 'sine', 0.5, 0.04);
      setTimeout(() => this.playChime(659.25, 'sine', 0.5, 0.04), 120);
      setTimeout(() => this.playChime(783.99, 'sine', 0.7, 0.05), 240);
    }
  };
  ...
  if (audioBtn) {
    audioBtn.addEventListener('click', () => {
      ...
      audioBtn.classList.toggle('is-playing', !AudioEngine.muted);
      if (!AudioEngine.muted) AudioEngine.playAtmosphereChime();
    });
  }
  ...
  /* ---------------------------------------------------------------------
     2b. VOLUMETRIC GOLD DUST PARTICLE SYSTEM (Alta Costura & Luxo)
     ---------------------------------------------------------------------
     Partículas de poeira de ouro 3D que flutuam suavemente pelo espaço,
     reagindo organicamente ao mouse e ao scroll. Dá um aspecto imediato de
     filme de alta perfumaria/joalheria nos primeiros 5 segundos.
     --------------------------------------------------------------------- */
  const particleCount = isMobile ? 90 : 250;
  const particleGeo = new THREE.BufferGeometry();
  const particlePositions = new Float32Array(particleCount * 3);
  const particleSpeeds = new Float32Array(particleCount * 3);

  for (let i = 0; i < particleCount; i++) {
    particlePositions[i * 3 + 0] = (Math.random() - 0.5) * 24;
    particlePositions[i * 3 + 1] = (Math.random() - 0.5) * 24;
    particlePositions[i * 3 + 2] = (Math.random() - 0.5) * 10 - 1;

    particleSpeeds[i * 3 + 0] = (Math.random() - 0.5) * 0.004;
  ...
  pGrad.addColorStop(0.3, 'rgba(227, 196, 141, 0.7)');
  pGrad.addColorStop(0.7, 'rgba(184, 154, 122, 0.2)');
  pGrad.addColorStop(1, 'rgba(184, 154, 122, 0.0)');
  ...
  const goldParticles = new THREE.Points(particleGeo, particleMat);
  ...
  function renderFrame() {
    ...
    // Animação contínua da poeira de ouro volumétrica 3D
    if (goldParticles && goldParticles.geometry.attributes.position) {
      const posArr = goldParticles.geometry.attributes.position.array;
      for (let pIdx = 0; pIdx < particleCount; pIdx++) {
        posArr[pIdx * 3 + 1] += particleSpeeds[pIdx * 3 + 1];
        posArr[pIdx * 3 + 0] += Math.sin(time * 0.4 + pIdx) * 0.002 + pointerWorld.x * 0.0004;

        if (posArr[pIdx * 3 + 1] > 12) {
          posArr[pIdx * 3 + 1] = -12;
          posArr[pIdx * 3 + 0] = (Math.random() - 0.5) * 24;
        }
      }
      goldParticles.geometry.attributes.position.needsUpdate = true;
      goldParticles.rotation.y = time * 0.015;
    }
  ...
  /* =====================================================================
     11. NOVAS FUNCIONALIDADES INTERATIVAS — Cardápio, Simulador & FAQ
     ===================================================================== */
  
  // A. Filtros do Cardápio Autoral
  const filterBtns = document.querySelectorAll('.filter-btn');
  const cakeCards = document.querySelectorAll('.cake-card');

  if (filterBtns.length && cakeCards.length) {
  ...
  const summaryTextEl = document.getElementById('builder-summary-text');
  const btnSendWhatsapp = document.getElementById('btn-send-whatsapp');

  function updateCakeBuilder() {
    if (!builderForm) return;

    const sizeInput = builderForm.querySelector('input[name="size"]:checked');
    const doughInput = builderForm.querySelector('input[name="dough"]:checked');
    const fillingInput = builderForm.querySelector('input[name="filling"]:checked');
    const styleInput = builderForm.querySelector('input[name="style"]:checked');

    if (!sizeInput || !doughInput || !fillingInput || !styleInput) return;

    // Highlight selected option pills
    builderForm.querySelectorAll('.option-pill').forEach(pill => {
    ...
    const basePrice = parseFloat(sizeInput.getAttribute('data-price') || 0);
    const extraFillingPrice = parseFloat(fillingInput.getAttribute('data-extra') || 0);
    const totalPrice = basePrice + extraFillingPrice;

    const yieldText = sizeInput.getAttribute('data-yield');
    const sizeVal = sizeInput.value;
    const doughVal = doughInput.value;
    const fillingVal = fillingInput.value;
    ...
    if (summaryTextEl) {
      summaryTextEl.textContent = `Tamanho ${sizeVal} (${yieldText}) • ${doughVal} • ${fillingVal} • ${styleVal}`;
    }

    if (btnSendWhatsapp) {
      const message = `Olá Mary! Montei meu bolo no site e gostaria de encomendar:%0A%0A` +
        `• *Tamanho:* ${sizeVal} (${yieldText})%0A` +
        `• *Massa:* ${doughVal}%0A` +
        `• *Recheio:* ${fillingVal}%0A` +
        `• *Estilo:* ${styleVal}%0A` +
        `• *Estimativa:* R$ ${totalPrice.toFixed(2).replace('.', ',')}%0A%0A` +
        `Poderia confirmar a disponibilidade para a minha data?`;

      btnSendWhatsapp.setAttribute('href', `https://wa.me/5515999999999?text=${message}`);
    }
  }

  if (builderForm) {
    builderForm.addEventListener('change', updateCakeBuilder);
    updateCakeBuilder(); // Inicializa
  }
  ...
  // D. Degustação Digital (Troca de Atmosfera 3D / WebGL)
  const flavorCards = document.querySelectorAll('.flavor-card');
  const atmospheres = {
    champagne: {
      baseColor: new THREE.Color(0xF5F2ED),
      goldColor: new THREE.Color(0xB89A7A),
      keyLight: new THREE.Color(0xffe6c2),
      rimLight: new THREE.Color(0xd9b878)
    },
    redvelvet: {
      baseColor: new THREE.Color(0xF8F0EE),
      goldColor: new THREE.Color(0x9E2A2B),
      keyLight: new THREE.Color(0xffc2c4),
      rimLight: new THREE.Color(0xc9714c)
    },
    pistache: {
      baseColor: new THREE.Color(0xF2F5ED),
      goldColor: new THREE.Color(0x7A9A60),
      keyLight: new THREE.Color(0xe6ffc2),
      rimLight: new THREE.Color(0xa3c48d)
    }
  };
  ...
  flavorCards.forEach(card => {
    card.addEventListener('click', () => {
      ...
      if (config && typeof bgUniforms !== 'undefined') {
        AudioEngine.playAtmosphereChime();

        gsap.to(bgUniforms.uBaseColor.value, {
        ...
        gsap.to(bgUniforms.uGoldColor.value, {
          r: config.goldColor.r,
          g: config.goldColor.g,
          b: config.goldColor.b,
          duration: 1.2,
          ease: 'power2.out'
        });
