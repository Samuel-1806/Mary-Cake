...
(() => {
  ...
  tl.to('#hero-logo-solid', { opacity: 0, scale: 0.9, duration: 0.6, ease: 'power3.in' }, 0.3)
    ...
    // ---- SEÇÃO 2 (Ateliê): giro lento e contínuo no eixo Y — um objeto
    // de joalheria girando devagar em sua vitrine, nunca um "spin"
    // mecânico — enquanto desliza suavemente para a direita, cedendo a
    // metade esquerda da tela ao texto.
    .to(phoneCenter.rotation, { y: '+=' + Math.PI * 1.15, duration: 2, ease: 'sine.inOut' }, 3)
    .to(phoneCenter.position, { x: 2.1, y: -0.1, z: -0.5, duration: 2, ease: 'sine.inOut' }, 3)

    // ---- SEÇÃO 3 (Experiência Digital): o celular "pousa" ao lado do
    // ecosystem-card — o card fica ancorado na coluna 2 (direita, grid
    // `justify-self: end` — ver styles.css), então o celular assume a
    // posição espelhada, à esquerda, quase encostado na borda do card,
    // como se o estivesse sustentando/apresentando — sem nunca cruzar
    // por cima do texto (coluna 1) nem do card (z-index 11, sempre à
    // frente do canvas).
    .to(phoneCenter.position, { x: -0.55, y: -0.08, z: -0.1, duration: 1.4, ease: 'power3.inOut' }, 5.1)
    .to(phoneCenter.rotation, { x: 0.02, y: 0.22, z: -0.015, duration: 1.4, ease: 'power3.inOut' }, 5.1)

    // ---- INTERVALO NEUTRO: a partir daqui, o scroll passa pela seção
    // dedicada "Telas do App" (100% DOM/2D — ver index.html + a IIFE
    // "TELAS DO APP" mais abaixo). O único celular 3D não precisa animar
    // nada aqui: fica parado e é discretamente esmaecido pela própria
    // `fadeLegacyPhoneClones()` daquela seção assim que ela entra em
    // cena, para nunca competir com o showcase 2D em primeiro plano.

    // ---- SEÇÃO 4 (Jornada do Pedido) — RESOLUÇÃO DE CONFLITO: o texto
    // aqui usa `.copy-center` (grid-column 1/3, largura total — ver
    // styles.css), então o celular precisa sair COMPLETAMENTE da frente
    // do conteúdo. Ele mergulha para o fundo da cena (eixo Z) e desliza
    // discretamente para fora do centro óptico (eixo X), perdendo
    // opacidade de material junto — reforça o esmaecimento do canvas
    // que já existe mais abaixo (7a/legibilidade).
    .to(camera.position, { x: 0, y: 0.12, z: 7.6, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(camera.rotation, { x: 0, duration: 1.6 }, 9.3)
    .to(phoneCenter.position, { x: 2.6, y: -0.2, z: -3.8, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(phoneCenter.rotation, { x: 0, y: '+=0.7', duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(opacityCenter, { v: 0.08, duration: 1.3, ease: 'power2.inOut', onUpdate: () => setPhoneOpacity(phoneCenter, opacityCenter.v) }, 9.3)

    // ---- SEÇÃO 5 (Nosso Processo / Compromisso com o Sabor): também
    // `.copy-center` — o celular permanece "fora de cena" (mesma tática:
    // fundo + lateral + baixa opacidade), só reequilibrando a composição
    // para o lado oposto, num deslocamento lento e contínuo que prepara
    // a convergência central da seção seguinte.
    .to(camera.position, { x: 0, y: 0.04, z: 8.2, duration: 1.8, ease: 'power3.inOut' }, 11.1)
    .to(phoneCenter.position, { x: -2.6, y: 0.1, z: -4, duration: 1.8, ease: 'sine.inOut' }, 11.1)
    ...
    // Parte já quase invisível (opacidade ~0.08, herdada da Seção 4/5),
    ...
    .to(phoneCenter.position, { x: 0, y: 0, z: -5, duration: 1.8, ease: 'power3.in' }, 13.0)
    .to(phoneCenter.rotation, { y: '+=3.2', duration: 1.8, ease: 'power3.in' }, 13.0)
    .to(phoneCenter.scale, { x: 0, y: 0, z: 0, duration: 1.8, ease: 'power3.in' }, 13.0)
    .to(opacityCenter, {
      v: 0, duration: 1.6, ease: 'power3.in',
      onUpdate: () => setPhoneOpacity(phoneCenter, opacityCenter.v)
    }, 13.0)

    // ---- SEÇÃO 7 (Download): câmera volta à posição neutra e o
    // celular RETORNA à cena — um único gesto final, elegante e
    // discreto, "apresentando" o QR Code em vez de competir com ele:
    // pousa ligeiramente à esquerda, levemente inclinado em direção ao
    // card, como se o estivesse entregando ao visitante.
    .to(camera.position, { x: 0, y: 0, z: 9, duration: 1.8, ease: 'power3.inOut' }, 14.9)
    .set(phoneCenter.position, { x: 0, y: 0, z: -3.4 }, 16.6)
    .set(phoneCenter.rotation, { x: 0, y: 0, z: 0 }, 16.6)
    .to(phoneCenter.position, { x: -1.85, y: -0.05, z: 0.35, duration: 1.7, ease: 'power3.out' }, 16.7)
    .to(phoneCenter.rotation, { y: 0.32, x: 0.02, duration: 1.7, ease: 'power3.out' }, 16.7)
    .to(phoneCenter.scale, { x: 0.86, y: 0.86, z: 0.86, duration: 1.7, ease: 'power3.out' }, 16.7)
    .to(opacityCenter, {
