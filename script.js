...
(() => {
  ...
  function buildSmartphone(textureUrl) {
    ...
    const placeholderMat = new THREE.MeshBasicMaterial({ color: 0x0c0c0d, transparent: true, opacity: 1 });
    ...
    group.userData.materials = [bodyMat, glassMat, camBumpMat]; // screen.material trocado dinamicamente, tratado à parte
    return group;
  }
  ...
  /* ---------------------------------------------------------------------
     3. LOGO DE ABERTURA — plano simples com a textura do logo, flutuando
     no centro da cena. No primeiro scroll, faz crossfade com o celular
     central: dissolve + encolhe enquanto o chassi nasce e gira até a pose.
     --------------------------------------------------------------------- */
  let logoMesh = null;

  function buildLogoMesh(imageUrl, callback) {
    ...
    // Flutuação suave de entrada (some do nada -> aparece com leve "respiro")
    mesh.scale.setScalar(0.001);
    gsap.to(mesh.scale, { x: 1, y: 1, z: 1, duration: 1.6, delay: 0.15, ease: 'power3.out' });

    callback(mesh);
  }

  buildLogoMesh('logo-mary-cake.png', (mesh) => {
    logoMesh = mesh;
  });
  ...
  function handlePointerMove(clientX, clientY) {
    ...
    // Spotlight dourado em CSS acompanha o cursor, revelando o relevo do fundo
    if (spotlightEl) {
      const mxPct = (clientX / window.innerWidth) * 100;
      const myPct = (clientY / window.innerHeight) * 100;
      spotlightEl.style.setProperty('--mx', mxPct + '%');
      spotlightEl.style.setProperty('--my', myPct + '%');
    }
  ...
  tl.to({}, { duration: 1 }, 0) // placeholder de tempo para sincronizar o crossfade abaixo
    .add(() => {
      if (logoMesh) {
        gsap.to(logoMesh.material, { opacity: 0, duration: 1, ease: 'power2.out' });
        gsap.to(logoMesh.scale, { x: 0.3, y: 0.3, z: 0.3, duration: 1, ease: 'power2.in' });
      }
    }, 0)
    ...
    .add(() => {
      gsap.to('.phone-tag', { opacity: 1, y: 0, duration: 0.9, stagger: 0.15, ease: 'power2.out' });
    }, 3.6)
    .add(() => {
      gsap.to('.phone-tag', { opacity: 0, y: 14, duration: 0.5, ease: 'power2.in' });
    }, 4.7)
    ...
    .add(() => {
      [phoneCenter, phoneLeft, phoneRight].forEach((p) => {
        gsap.to({ v: 1 }, {
          v: 0,
          duration: 1.6,
          ease: 'power2.in',
          onUpdate: function () { setPhoneOpacity(p, this.targets()[0].v); }
        });
      });
    }, 8.7);
