...
(() => {
  ...
  function buildSmartphone(textureFile) {
    ...
    const camPlateSize = 0.62;
    const camPlateRadius = 0.16;
    ...
    function buildLens(radius, protrusion) {
      ...
      const glassMat = new THREE.MeshStandardMaterial({
        color: 0x0a0d16, metalness: 0.9, roughness: 0.08, transparent: true, opacity: 1,
    ...
    const mainLens = buildLens(0.148, 0.022);
    mainLens.position.x = -0.108;
    mainLens.position.y = 0;
    camModule.add(mainLens);

    const wideLens = buildLens(0.105, 0.016);
    wideLens.position.x = 0.148;
    wideLens.position.y = 0.148;
    camModule.add(wideLens);

    const teleLens = buildLens(0.105, 0.016);
    teleLens.position.x = 0.148;
    teleLens.position.y = -0.148;
    camModule.add(teleLens);

    // Flash (canto oposto às lentes menores)
    const flashGeo = new THREE.CircleGeometry(0.035, 16);
    const flashMat = new THREE.MeshStandardMaterial({
      color: 0xe9dfc4, metalness: 0.15, roughness: 0.45, transparent: true, opacity: 1,
      polygonOffset: true, polygonOffsetFactor: -2, polygonOffsetUnits: -2
    });
    const flash = new THREE.Mesh(flashGeo, flashMat);
    flash.position.set(-0.189, 0.189, 0.016);
    camModule.add(flash);

    // Microfone (pontinho escuro colado à lente principal)
    const micGeo = new THREE.CircleGeometry(0.022, 12);
    ...
    mic.position.set(-0.162, -0.202, 0.016);
    camModule.add(mic);
