...
(() => {
  ...
  function buildSmartphone(textureFile) {
    ...
    const camPlateSize = 0.46;
    const camPlateRadius = 0.12;
    ...
    const mainLens = buildLens(0.11, 0.016);
    mainLens.position.x = -0.08;
    mainLens.position.y = 0;
    camModule.add(mainLens);

    const wideLens = buildLens(0.078, 0.012);
    wideLens.position.x = 0.11;
    wideLens.position.y = 0.11;
    camModule.add(wideLens);

    const teleLens = buildLens(0.078, 0.012);
    teleLens.position.x = 0.11;
    teleLens.position.y = -0.11;
    camModule.add(teleLens);

    // Flash (canto oposto às lentes menores)
    const flashGeo = new THREE.CircleGeometry(0.026, 16);
    const flashMat = new THREE.MeshStandardMaterial({
      color: 0xe9dfc4, metalness: 0.15, roughness: 0.45, transparent: true, opacity: 1,
      polygonOffset: true, polygonOffsetFactor: -2, polygonOffsetUnits: -2
    });
    const flash = new THREE.Mesh(flashGeo, flashMat);
    flash.position.set(-0.14, 0.14, 0.012);
    camModule.add(flash);

    // Microfone (pontinho escuro colado à lente principal)
    const micGeo = new THREE.CircleGeometry(0.016, 12);
    ...
    mic.position.set(-0.12, -0.15, 0.012);
    camModule.add(mic);
