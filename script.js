...
(() => {
  ...
  const bgVertexShader = `
    ...
    // sem retas: tudo curva contínua.
    float clothWave(vec2 p, float t) {
      return sin(p.x * 0.5 + t * 0.45) * 0.24
           + cos(p.y * 0.42 - t * 0.36) * 0.19
           + sin((p.x + p.y) * 0.3 + t * 0.58) * 0.12
           + sin(p.x * 0.15 - p.y * 0.21 + t * 0.16) * 0.14; // dobra larga e lenta (drapeado)
    }

    // Relevo tátil sob o cursor: elevação ampla e suave (smoothstep),
    // nunca uma cratera nem um facetado — leitura de dedo pressionando seda.
    float clothBump(vec2 p, vec2 mouse, float active) {
      float dist = distance(p, mouse);
      float bump = smoothstep(3.1, 0.0, dist) * active;
      float bumpSoft = smoothstep(4.4, 0.0, dist) * active;
      return bump * 0.85 + bumpSoft * 0.22;
    }
    ...
    void main() {
      ...
      // É isso que faz a luz "entender" as dobras — sem isso o pano fica
      // deslocado em Z mas chapado na sombra, sem volume de tecido real.
      float eps = 0.4;
      ...
      vBump = clamp(clothBump(p, uMouse, uMouseActive) / 1.07, 0.0, 1.0);
  ...
  const bgFragmentShader = `
    ...
    void main() {
      ...
      // Sheen especular de seda: destaque estreito e quente nas cristas
      // das dobras, exatamente como fibra têxtil captando luz direcional.
      vec3 viewDir = vec3(0.0, 0.0, 1.0);
      vec3 halfDir = normalize(lightDir + viewDir);
      float specular = pow(max(dot(N, halfDir), 0.0), 26.0);
      ...
      float sheen = smoothstep(0.25, 0.95, sin(fold + uTime * 0.12) * 0.5 + 0.5);
      float fold2 = vWave * 1.8 - vUvLocal.x * 1.1 - uTime * 0.07;
      float sheen2 = smoothstep(0.35, 0.96, sin(fold2) * 0.5 + 0.5);

      // Tons derivados SOMENTE das duas cores da paleta: vale da dobra
      // (mistura discreta em direção ao dourado) e crista luminosa
      // (dourado clareado em direção ao branco do próprio tecido).
      vec3 shadowTone = mix(uBaseColor, uGoldColor, 0.30);
      vec3 highlightTone = mix(uGoldColor, vec3(1.0), 0.55);

      vec3 clothColor = mix(shadowTone, uBaseColor, diffuse);
      clothColor = mix(clothColor, uGoldColor, sheen * 0.18 + sheen2 * 0.08);
      clothColor += highlightTone * specular * 0.5;

      float bumpStrength = clamp(vBump, 0.0, 1.0);
      clothColor = mix(clothColor, highlightTone, bumpStrength * 0.55);

      // Alpha: base sempre visível o suficiente para ler como seda viva,
      // mas nunca opaca — o brilho especular e o toque do cursor "acendem"
      // a superfície sem nunca virar mancha sólida.
      float baseAlpha = 0.18 + (1.0 - diffuse) * 0.10;
      float sheenAlpha = sheen * 0.10 + sheen2 * 0.05;
      float specAlpha = specular * 0.35;
      float bumpAlpha = bumpStrength * 0.42;

      float alpha = clamp(baseAlpha + sheenAlpha + specAlpha + bumpAlpha, 0.0, 0.62);
