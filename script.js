...
(() => {
  ...
  /* ---------------------------------------------------------------------
     0e. SOM AMBIENTE (OPT-IN) — micro-cliques/hovers sutilíssimos em
     botões e links, reforçando a sensação de luxo sem incomodar quem
     prefere silêncio. Regras:
       - Começa DESLIGADO. Só liga com um clique explícito do visitante
         no ícone (nada de autoplay).
       - O AudioContext só é criado dentro desse gesto de clique, porque
         é exatamente isso que os navegadores exigem para permitir áudio.
       - Nada de arquivos .mp3/.wav: os sons são sintetizados na hora via
         Web Audio API (osciladores + envelope de ganho), então não há
         asset extra para hospedar — só um "tic" e um "toc" muito breves
         e muito baixos.
       - Preferência persiste em localStorage entre visitas.
     --------------------------------------------------------------------- */
  (() => {
    const toggleBtn = document.getElementById('sound-toggle');
    if (!toggleBtn) return;

    const STORAGE_KEY = 'maryCake:soundEnabled';
    let soundEnabled = false;
    let audioCtx = null;

    // Lê a preferência salva só pra refletir o estado visual do ícone;
    // o AudioContext em si continua sendo criado apenas no primeiro
    // clique (gesto do usuário), nunca automaticamente no load.
    try {
    ...
    // Um "tic"/"toc" curtíssimo: oscilador senoidal com envelope de
    // ganho que sobe e desce em poucas dezenas de ms — nunca um beep
    // óbvio, mais uma textura discreta, no espírito "seda fina" do
    // resto do site.
    function playTone({ freq, decay, gain }) {
      ...
      osc.frequency.exponentialRampToValueAtTime(Math.max(60, freq * 0.72), ctx.currentTime + decay);

      gainNode.gain.setValueAtTime(0, ctx.currentTime);
      gainNode.gain.linearRampToValueAtTime(gain, ctx.currentTime + 0.008);
    ...
    const playHoverTick = () => playTone({ freq: 1500, decay: 0.06, gain: 0.02 });
    const playClickTock = () => playTone({ freq: 720, decay: 0.09, gain: 0.035 });

    toggleBtn.addEventListener('click', () => {
      soundEnabled = !soundEnabled;
      toggleBtn.setAttribute('aria-pressed', String(soundEnabled));
      toggleBtn.setAttribute('aria-label', soundEnabled
        ? 'Silenciar sons sutis de interface'
        : 'Ativar sons sutis de interface');
      try { localStorage.setItem(STORAGE_KEY, String(soundEnabled)); } catch (err) { /* ambiente sem storage — segue sem persistir */ }

      if (soundEnabled) {
        ensureAudioContext();
        playClickTock();
        toggleBtn.classList.remove('is-pulsing');
        void toggleBtn.offsetWidth; // reinicia a animação de pulso mesmo em cliques seguidos
        toggleBtn.classList.add('is-pulsing');
      }
    });

    // Delegação única de evento — mesma lista de seletores usada pelo
    // cursor customizado, cobrindo nav, CTAs e cards clicáveis.
    const SOUND_HOVER_SELECTOR = '.site-nav a, a.btn-primary, a.btn-header, a.btn-ghost, .brand';
    const SOUND_CLICK_SELECTOR = 'a, button';

    document.addEventListener('mouseover', (e) => {
      const target = e.target.closest(SOUND_HOVER_SELECTOR);
      if (target) playHoverTick();
    });

    document.addEventListener('click', (e) => {
      if (e.target.closest('#sound-toggle')) return; // já tem seu próprio som acima
      const target = e.target.closest(SOUND_CLICK_SELECTOR);
      if (target) playClickTock();
    });
  })();
  ...
  const bgVertexShader = `
    ...
    float clothBump(vec2 p, vec2 mouse, float active) {
      float dist = distance(p, mouse);
      float bump = smoothstep(1.6, 0.0, dist) * active;
      float bumpSoft = smoothstep(2.6, 0.0, dist) * active;
      return bump * 0.55 + bumpSoft * 0.18;
    }

    float clothHeight(vec2 p, float t, vec2 mouse, float active) {
      return clothWave(p, t) + clothBump(p, mouse, active);
    }
  ...
  const PHONE_CORNER_RADIUS = 0.22;
  ...
  (function setupTelasShowcase() {
    ...
    /* -----------------------------------------------------------------
       CORREÇÃO DO "6 CELULARES": os 3 celulares 3D (phoneCenter/Left/
       Right, Three.js) que a seção anterior ("Experiência Digital") usa
       ficam parados e VISÍVEIS bem atrás desta seção — antes, só a
       opacidade do <canvas> baixava (0.12), então esses 3 "fantasmas"
       continuavam por trás dos 3 celulares novos em DOM, sobrepostos
       quase pixel a pixel. Agora escondemos esses clones 3D de verdade
       (fade da opacidade do MATERIAL, não do canvas) sempre que a
       formação DOM está em cena, e os trazemos de volta assim que ela
       sai — sem depender/conflitar com a opacidade do canvas.
       ----------------------------------------------------------------- */
    function fadeLegacyPhoneClones(hidden) {
      [phoneCenter, phoneLeft, phoneRight].forEach((grp) => {
        if (!grp) return;
    ...
    /* -----------------------------------------------------------------
       O 'REVEZAMENTO DE ESTADOS' — equivalente do updateCellState()
       pedido no briefing, operando sobre os 3 <div class="telas-phone">
    ...
    // prefere navegar direto em vez de rolar. Também sincroniza a store
    // 3D (cellStore, seção 3c) quando presente.
    dots.forEach((dot) => {
  ...
  tl.to('#hero-logo-solid', { opacity: 0, scale: 0.9, duration: 0.6, ease: 'power3.in' }, 0.3)
    ...
    // Entrada "majestosa": o celular não só cresce a partir do morph de
    // grânulos, como também SOBE de um pouco abaixo até a pose final e
    // ganha opacidade de material gradualmente (0 -> 1) — em vez de
    // aparecer already-opaco no instante em que scale sai de 0. O eixo X
    // permanece o de PHONE_TARGET_POS (levemente à direita do centro
    // óptico), que já cai na metade da tela livre de texto, já que
    // `.hero-copy` ocupa só a coluna 1/2 da grid (ver styles.css).
    .fromTo(phoneCenter.position,
      { y: PHONE_TARGET_POS.y - 0.85 },
      { y: PHONE_TARGET_POS.y, duration: 0.95, ease: 'power3.out' }, 2.3)
    .fromTo(phoneCenter.scale,
      { x: 0.001, y: 0.001, z: 0.001 },
      { x: 1, y: 1, z: 1, duration: 0.7, ease: 'power3.out' }, 2.3)
    .fromTo(opacityCenter,
      { v: 0 },
      { v: 1, duration: 0.75, ease: 'power2.out', onUpdate: () => setPhoneOpacity(phoneCenter, opacityCenter.v) }, 2.3)
    ...
    // ---- SEÇÃO 3 (Experiência Digital): os 3 celulares emolduram o
    // ecosystem-card. O card fica ancorado na coluna 2 (direita, grid
    // `justify-self: end` — ver styles.css), então o trio inteiro é
    // deslocado um pouco à ESQUERDA do centro óptico (em vez de x:0
    // cravado no meio da tela): o phoneCenter fica quase atrás do card,
    // phoneRight "pousa" rente à borda esquerda dele (moldura), e
    // phoneLeft equilibra a composição do lado do texto, sem cobri-lo.
    .to(phoneCenter.position, { x: -0.35, y: -0.05, z: -0.2, duration: 1.4, ease: 'power3.inOut' }, 5.1)
    .to(phoneCenter.rotation, { x: 0, y: -0.18, z: 0, duration: 1.4 }, 5.1)
    .set(phoneLeft, { visible: true }, 5.3)
    .set(phoneRight, { visible: true }, 5.3)
    .fromTo(phoneLeft.position, { x: -5, y: 0.3, z: -2 }, { x: -2.15, y: -0.2, z: -0.7, duration: 1.4, ease: 'power3.out' }, 5.3)
    .fromTo(phoneRight.position, { x: 5, y: -0.3, z: -2 }, { x: 1.35, y: -0.1, z: -0.4, duration: 1.4, ease: 'power3.out' }, 5.3)
    .to(phoneLeft.rotation, { y: 0.42, x: 0.05, duration: 1.4 }, 5.3)
    .to(phoneRight.rotation, { y: -0.3, x: 0.05, duration: 1.4 }, 5.3)
    ...
    // ---- SEÇÃO 4 (Jornada do Pedido) — RESOLUÇÃO DE CONFLITO: o texto
    // aqui usa `.copy-center` (grid-column 1/3, largura total — ver
    // styles.css), então os celulares precisam sair COMPLETAMENTE da
    // frente do conteúdo, não só se deslocar para um lado. Aplicamos as
    // duas táticas pedidas ao mesmo tempo: os laterais deslizam pra fora
    // no eixo X (bem além das bordas da tela) e o central mergulha para
    // o fundo no eixo Z, todos perdendo opacidade de material junto —
    // reforça o esmaecimento do canvas que já existe mais abaixo (6e).
    .to(camera.position, { x: 0, y: 0.15, z: 7.6, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(camera.rotation, { x: 0, duration: 1.6 }, 9.3)
    .to(phoneCenter.position, { x: 0, y: -0.15, z: -3.6, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(phoneCenter.rotation, { x: 0, y: '+=0.5', duration: 1.6 }, 9.3)
    .to(phoneLeft.position, { x: -6.8, z: -2.8, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(phoneRight.position, { x: 6.8, z: -2.8, duration: 1.6, ease: 'power3.inOut' }, 9.3)
    .to(opacityCenter, { v: 0.1, duration: 1.3, ease: 'power2.inOut', onUpdate: () => setPhoneOpacity(phoneCenter, opacityCenter.v) }, 9.3)
    .to(opacityLeft, { v: 0.1, duration: 1.3, ease: 'power2.inOut', onUpdate: () => setPhoneOpacity(phoneLeft, opacityLeft.v) }, 9.3)
    .to(opacityRight, { v: 0.1, duration: 1.3, ease: 'power2.inOut', onUpdate: () => setPhoneOpacity(phoneRight, opacityRight.v) }, 9.3)

    // ---- SEÇÃO 5 (Nosso Processo / Compromisso com o Sabor): também
    // `.copy-center`, então os celulares permanecem "fora de cena"
    // (mesma tática — X para os laterais, Z+opacidade para o central),
    // só reequilibrando levemente a composição para a transição
    // seguinte, nunca voltando à frente do texto.
    .to(camera.position, { x: 0, y: 0.05, z: 8.2, duration: 1.8, ease: 'power3.inOut' }, 11.1)
    .to(phoneCenter.position, { x: -0.3, y: 0, z: -3.9, duration: 1.8, ease: 'power2.inOut' }, 11.1)
    .to(phoneLeft.position, { x: -7.4, y: 0.4, z: -3.2, duration: 1.8, ease: 'power2.inOut' }, 11.1)
    .to(phoneRight.position, { x: 7.4, y: -0.3, z: -3.2, duration: 1.8, ease: 'power2.inOut' }, 11.1)
    ...
    // Partem já quase invisíveis (opacidade ~0.1, herdada da Seção 4/5),
    ...
    .to([phoneCenter.position, phoneLeft.position, phoneRight.position], { x: 0, y: 0, z: '-=1.4', duration: 1.8, ease: 'power3.in' }, 13.0)
    ...
    // ---- SEÇÃO 7 (Download): câmera volta à posição neutra e, em
    // seguida, phoneLeft/phoneRight RETORNAM à cena de forma simétrica,
    // ladeando o espaço do QR Code (phoneCenter permanece dissolvido —
    // o QR é o protagonista único no centro). Primeiro um `.set` os
    // reposiciona (ainda invisíveis, escala 0) atrás da câmera, depois
    // sobem em espelho para as laterais com scale/opacity/rotation
    // simétricos — mesma duração e ease dos dois lados, para reforçar
    // a simetria pedida no briefing.
    .to(camera.position, { x: 0, y: 0, z: 9, duration: 1.8, ease: 'power3.inOut' }, 14.9)
    .set([phoneLeft.position, phoneRight.position], { x: 0, y: 0, z: -3.2 }, 16.6)
    .set([phoneLeft.rotation, phoneRight.rotation], { x: 0, y: 0, z: 0 }, 16.6)
    .to(phoneLeft.position, { x: -2.5, y: -0.1, z: 0.3, duration: 1.6, ease: 'power3.out' }, 16.7)
    .to(phoneRight.position, { x: 2.5, y: -0.1, z: 0.3, duration: 1.6, ease: 'power3.out' }, 16.7)
    .to(phoneLeft.rotation, { y: 0.48, duration: 1.6, ease: 'power3.out' }, 16.7)
    .to(phoneRight.rotation, { y: -0.48, duration: 1.6, ease: 'power3.out' }, 16.7)
    .to([phoneLeft.scale, phoneRight.scale], { x: 0.82, y: 0.82, z: 0.82, duration: 1.6, ease: 'power3.out' }, 16.7)
    .to(opacityLeft, {
  ...
  if (!prefersReducedMotion) {
    ...
    const textBlocks = gsap.utils.toArray(
      '.section-atelie .copy-block, .section-experiencia .copy-block, .section-download .copy-block'
    );
