...
function initFabricBackground() {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      /* ruído 2D leve — só para o grão fino entre as dobras */
      float hash(vec2 p){
      ...
      /* uma "dobra" que se propaga a partir de um ponto fixo (canto, lateral,
         centro), como se o pano tivesse sido puxado ali — cada uma com sua
         própria frequência/velocidade/fase, pra nunca sincronizarem. */
      float ridge(vec2 p, vec2 anchor, float freq, float speed, float seed){
        vec2 d = p - anchor;
        float r = length(d);
        float wave = sin(r * freq - uTime * speed + seed);
        float falloff = exp(-r * r * 0.6);
        return wave * falloff;
      }

      float heightField(vec2 p, float aspect){
        float h = 0.0;
        h += ridge(p, vec2(0.06, 0.10), 2.6, 0.30, 0.0)              * 0.42;
        h += ridge(p, vec2(aspect - 0.06, 0.12), 2.2, 0.24, 2.4)     * 0.40;
        h += ridge(p, vec2(0.10, 0.90), 2.4, 0.27, 4.8)              * 0.40;
        h += ridge(p, vec2(aspect - 0.08, 0.88), 2.8, 0.22, 7.2)     * 0.42;
        h += ridge(p, vec2(aspect * 0.5, 0.5), 1.7, 0.18, 9.6)       * 0.50;
        h += ridge(p, vec2(aspect * 0.22, 0.5), 3.1, 0.33, 12.0)     * 0.30;
        h += ridge(p, vec2(aspect * 0.78, 0.45), 2.9, 0.29, 14.4)    * 0.30;
        h += ridge(p, vec2(aspect * 0.5, 0.08), 3.3, 0.35, 16.8)     * 0.25;
        h += (noise(p * 46.0 + uTime * 0.015) - 0.5) * 0.05;
        return h * 0.55;
      }

      /* relevo extra sob o cursor — o "empurrão" do mouse no tecido */
      float mouseBump(vec2 p, vec2 mouse, float influence){
        float d = length(p - mouse);
        return exp(-d * d * 8.5) * influence;
      }
      ...
      void main(){
        ...
        /* deriva o relevo total (dobras + toque do mouse) por diferenças
           finitas, pra conseguir uma normal e sombrear como tecido de verdade */
        float eps = 0.0028;
        float h0 = heightField(auv, aspect)              + mouseBump(auv, mouseUv, uMouseInfluence) * 0.55;
        float hx = heightField(auv + vec2(eps, 0.0), aspect) + mouseBump(auv + vec2(eps, 0.0), mouseUv, uMouseInfluence) * 0.55;
        float hy = heightField(auv + vec2(0.0, eps), aspect) + mouseBump(auv + vec2(0.0, eps), mouseUv, uMouseInfluence) * 0.55;

        vec3 normal = normalize(vec3((h0 - hx) / eps, (h0 - hy) / eps, 2.0));

        vec3 lightDir = normalize(vec3(0.35, 0.55, 0.75));
        float ndotl = dot(normal, lightDir);
        float lightness = clamp(0.5 + ndotl * 0.55, 0.0, 1.0);

        /* pano branco/creme com cinza sutil dentro das dobras */
        vec3 shadowGray = vec3(0.800, 0.788, 0.762);
        vec3 highlight  = vec3(0.985, 0.979, 0.968);
        vec3 clothColor = mix(shadowGray, highlight, lightness);

        /* dourado só na área que o mouse está deformando */
        float bumpVal = mouseBump(auv, mouseUv, uMouseInfluence);
        vec3 gold = vec3(0.867, 0.706, 0.596);
        float goldAmount = clamp(bumpVal * 1.2, 0.0, 0.5);
        vec3 color = mix(clothColor, mix(clothColor, gold, 0.65), goldAmount);
