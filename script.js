...
(() => {
  ...
  const bgFragmentShader = `
    ...
    void main() {
      ...
      // lentamente por toda a malha, independente do mouse.
      ...
      // IMPORTANTE: a linha SEMPRE tem uma base visível (0.06), que não
      // depende da respiração — antes, a base ficava baixa demais (e
      // multiplicada por termos que podiam se aproximar de zero), o que
      // fazia a malha inteira sumir de vista. A respiração agora só
      // ADICIONA um brilho por cima dessa base, nunca a apaga.
      float baseAlpha = line * 0.06;
      float breatheAlpha = line * breathe * 0.05;
      float goldAlpha = line * bumpStrength * 0.62 + bumpStrength * 0.05;

      gl_FragColor = vec4(color, baseAlpha + breatheAlpha + goldAlpha);
    }
  `;
  ...
  tl.to('#hero-logo-solid', { opacity: 0, scale: 0.9, duration: 0.6, ease: 'power2.in' }, 0.3)
    ...
    .to(camera.position, { x: 0, y: 0, z: 9, duration: 1.8, ease: 'power2.inOut' }, 10.7)
