...
(() => {
  ...
  // Trava o scroll da página assim que o script inicia: a coreografia de
  // abertura (logo sólido -> desintegração -> montagem do celular) precisa
  // rodar sozinha, sem que o usuário consiga "pular" ela rolando a página
  // no meio do processo. É destravada em playIntroSequence() só quando a
  // intro termina de verdade (ver seção 4b, mais abaixo).
    ...
    if (assetsLoaded >= TOTAL_ASSETS) {
      gsap.delayedCall(0.35, () => {
        ...
        if (!introDispatched) {
          ...
          document.dispatchEvent(new Event('mary:assets-ready'));
        }
      });
    }
  }
  ...
  const bgFragmentShader = `
    ...
    void main() {
      // Grid minimalista: agora com deriva lenta e contínua no tempo — as
      // linhas deslizam devagar pela tela, como um tecido respirando.
      // Antes, a grade era calculada só em gl_FragCoord (pixel de tela) e
      // nunca reagia a nada além do mouse — por isso parecia 100% estática.
      vec2 drift = vec2(sin(uTime * 0.09) * 16.0, cos(uTime * 0.07) * 16.0);
      vec2 grid = (gl_FragCoord.xy + drift) / 26.0;
      ...
      // Respiração ambiente: um brilho quente muito sutil, pulsando
      // lentamente por toda a malha, independente do mouse — garante que
      // a "vida" da grade seja visível mesmo sem interação nenhuma.
      float breathe = clamp((vAmbient + 0.095) / 0.19, 0.0, 1.0);
      ...
      float baseAlpha = line * (0.035 + breathe * 0.03);
  ...
  /* ---------------------------------------------------------------------
     7. INTRO CINEMÁTICA — LOGO SÓLIDO -> DESINTEGRAÇÃO -> CELULAR
     ---------------------------------------------------------------------
     Antes, esse morph vivia DENTRO da timeline scrubada pelo scroll — ou
     seja, só rodava se e quando o usuário rolasse a página, na velocidade
     que ele rolasse (por isso ficava abrupto/"agressivo" em scrolls
     rápidos, e não tocava sozinho no carregamento).

     Agora é uma timeline GSAP independente, tocada automaticamente assim
     que os assets terminam de carregar (evento 'mary:assets-ready',
     disparado em onAssetLoaded — seção 0). Enquanto ela roda, a página
     fica com o scroll travado (também na seção 0), então a coreografia
     sempre tem a MESMA duração elegante, em segundos reais — nunca
     amarrada à velocidade de rolagem de quem está usando.
     --------------------------------------------------------------------- */
  const introTl = gsap.timeline({ paused: true });

  introTl
    // FASE 1 — logo sólido: os grânulos já nascem desenhando o contorno
    // cheio do logo (seção 4). Aqui é só uma pausa de contemplação antes
    // de qualquer movimento começar.
    .to({}, { duration: prefersReducedMotion ? 0.05 : 0.9 })

    // FASE 2 — desintegração: os grânulos se soltam da forma do logo e
    // flutuam, devagar, sem pressa.
    .to(granuleMorph, {
      scatter: 1,
      duration: prefersReducedMotion ? 0.05 : 1.3,
      ease: 'sine.inOut',
      onUpdate: updateGranules
    })

    // FASE 3 — remontagem: os grânulos convergem, com a mesma suavidade,
    // para os vértices reais do chassi do celular.
    .to(granuleMorph, {
      scatter: 0,
      t: 1,
      duration: prefersReducedMotion ? 0.05 : 1.9,
      ease: 'power2.inOut',
      onUpdate: updateGranules
    }, '-=0.15')

    // FASE 4 — cross-fade final: o celular real nasce enquanto os grânulos
    // se apagam. Sem "back.out"/bounce — troca elegante, nunca um solavanco.
    .set(phoneCenter, { visible: true }, '-=0.35')
    .fromTo(phoneCenter.scale,
      { x: 0.001, y: 0.001, z: 0.001 },
      { x: 1, y: 1, z: 1, duration: 0.9, ease: 'power3.out' }, '-=0.35')
    .to(granuleMat, { opacity: 0, duration: 0.7, ease: 'power2.in' }, '-=0.55')
    .set(logoGranules, { visible: false })
    .call(() => {
      // Só agora o scroll é destravado — a página "acorda" com o celular
      // já pronto na cena, pronto pra assumir a coreografia da seção 8.
  ...
  document.addEventListener('mary:assets-ready', () => introTl.play(0), { once: true });

  // Rede de segurança: se o evento de assets nunca disparar por algum
  // motivo (ex.: aba em segundo plano durante o load), garante que a
  // intro toque e o scroll destrave de qualquer forma.
  gsap.delayedCall(6.5, () => { if (introTl.progress() === 0) introTl.play(0); });

  /* ---------------------------------------------------------------------
     8. COREOGRAFIA DE SCROLL — GSAP ScrollTrigger, timeline única
     ---------------------------------------------------------------------
     ...
     Começa direto na SEÇÃO 2 (Ateliê): quando o scroll é liberado pela
     intro acima, o celular central já está montado e parado na pose de
     chegada — a coreografia de scroll assume a partir daí.
     --------------------------------------------------------------------- */
  ...
  tl.to(phoneCenter.rotation, { y: '+=' + (Math.PI * 2), duration: 2, ease: 'power1.inOut' }, 0)
    .to(phoneCenter.position, { x: 2.1, y: -0.1, z: -0.5, duration: 2 }, 0)

    // ---- SEÇÃO 3 (Experiência Digital): os 3 celulares surgem juntos no centro
    .to(phoneCenter.position, { x: 0, y: 0, z: 0, duration: 1.4, ease: 'power2.inOut' }, 2.1)
    .to(phoneCenter.rotation, { x: 0, y: -0.18, z: 0, duration: 1.4 }, 2.1)
    .set(phoneLeft, { visible: true }, 2.3)
    .set(phoneRight, { visible: true }, 2.3)
    .fromTo(phoneLeft.position, { x: -5, y: 0.3, z: -2 }, { x: -1.85, y: -0.15, z: -0.6, duration: 1.4, ease: 'power2.out' }, 2.3)
    .fromTo(phoneRight.position, { x: 5, y: -0.3, z: -2 }, { x: 1.85, y: -0.15, z: -0.6, duration: 1.4, ease: 'power2.out' }, 2.3)
    .to(phoneLeft.rotation, { y: 0.42, x: 0.05, duration: 1.4 }, 2.3)
    .to(phoneRight.rotation, { y: -0.42, x: 0.05, duration: 1.4 }, 2.3)
    .to('.phone-tag', { opacity: 1, y: 0, duration: 0.9, stagger: 0.15, ease: 'power2.out' }, 2.6)
    .to('.phone-tag', { opacity: 0, y: 14, duration: 0.5, ease: 'power2.in' }, 3.7)

    // ---- SEÇÃO 4 (Jornada do Pedido): macro zoom inclinado na tela central
    .to(camera.position, { x: 0.15, y: 0.85, z: 2.6, duration: 1.6, ease: 'power2.inOut' }, 4)
    .to(camera.rotation, { x: -0.32, duration: 1.6 }, 4)
    .to(phoneCenter.position, { x: 0, y: 0.1, z: 1, duration: 1.6 }, 4)
    .to(phoneCenter.rotation, { x: 0.08, y: 0, duration: 1.6 }, 4)
    .to([phoneLeft.position, phoneRight.position], { z: '-=0.6', duration: 1.6 }, 4)

    // ---- SEÇÃO 5 (Compromisso com o Sabor): texto fica à esquerda, celulares migram para a direita
    .to(camera.position, { x: 1.4, y: 0.1, z: 6.5, duration: 1.8, ease: 'power2.inOut' }, 5.8)
    .to(camera.rotation, { x: 0, duration: 1.8 }, 5.8)
    .to(phoneCenter.position, { x: 2.1, y: 0, z: -0.3, duration: 1.8 }, 5.8)
    .to(phoneCenter.rotation, { y: -0.35, duration: 1.8 }, 5.8)
    .to(phoneLeft.position, { x: 0.9, z: -2.2, duration: 1.8 }, 5.8)
    .to(phoneRight.position, { x: 3.8, z: -2.4, duration: 1.8 }, 5.8)
    ...
    .to(camera.position, { x: 0, y: 0, z: 9, duration: 1.8, ease: 'power2.inOut' }, 7.7)
    .to([phoneCenter.position, phoneLeft.position, phoneRight.position], { x: 0, y: 0, z: '-=2', duration: 1.8, ease: 'power2.in' }, 7.7)
    .to([phoneCenter.rotation, phoneLeft.rotation, phoneRight.rotation], { y: '+=3.2', duration: 1.8, ease: 'power2.in' }, 7.7)
    .to([phoneCenter.scale, phoneLeft.scale, phoneRight.scale], { x: 0, y: 0, z: 0, duration: 1.8, ease: 'power3.in' }, 7.7)
    .to(opacityCenter, {
  ...
  /* ---------------------------------------------------------------------
     9. RESIZE / RESPONSIVO — celulares reescalam no mobile sem cobrir texto
     --------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------
     9b. HEADER INTELIGENTE — esconde ao rolar pra baixo, reaparece
  ...
  /* ---------------------------------------------------------------------
     11. LINKS / CTAs — desktop mostra QR (com link real em HTML), mobile
     mostra botão de APK
     --------------------------------------------------------------------- */
