...
(() => {
  ...
  tl.to('#hero-logo-solid', { opacity: 0, scale: 0.9, duration: 0.6, ease: 'power3.in' }, 0.3)
    ...
    // ---- SEÇÃO 1 (Abertura — CLOSE-UP CINEMATOGRÁFICO): o celular nasce
    // do morph de grânulos já em plano MACRO — escala grande, deslocado
    // do centro e puxado para perto da câmera (z alto), como se
    // víssemos só um fragmento do aparelho preenchendo o quadro. rotationX/
    // rotationY partem de um ângulo mais cru/dramático, ainda não a pose
    // "de vitrine" final — essa vem só na próxima etapa (Afastamento).
    .fromTo(phoneCenter.position,
      { x: PHONE_TARGET_POS.x + 0.9, y: PHONE_TARGET_POS.y - 0.85, z: PHONE_TARGET_POS.z + 2.6 },
      { x: PHONE_TARGET_POS.x + 0.55, y: PHONE_TARGET_POS.y + 0.1, z: PHONE_TARGET_POS.z + 2.2, duration: 0.95, ease: 'power3.out' }, 2.3)
    .fromTo(phoneCenter.scale,
      { x: 0.001, y: 0.001, z: 0.001 },
      { x: 2.35, y: 2.35, z: 2.35, duration: 0.7, ease: 'power3.out' }, 2.3)
    .fromTo(phoneCenter.rotation,
      { x: PHONE_TARGET_ROT.x - 0.25, y: PHONE_TARGET_ROT.y - 0.4 },
      { x: PHONE_TARGET_ROT.x + 0.08, y: PHONE_TARGET_ROT.y + 0.15, duration: 0.95, ease: 'power3.out' }, 2.3)
    ...
    // ---- SEÇÃO 1B (Afastamento — "a câmera do diretor recua"): do
    // close-up extremo, o celular encolhe suavemente enquanto gira nos
    // eixos X e Y até assumir a pose elegante ao lado do título — uma
    // transição longa (1s de timeline) e macia, sem pressa, para dar o
    // "peso" de um comercial de alto padrão.
    .to(phoneCenter.scale, { x: 1, y: 1, z: 1, duration: 1.0, ease: 'power2.inOut' }, 3.0)
    .to(phoneCenter.position, {
      x: PHONE_TARGET_POS.x, y: PHONE_TARGET_POS.y, z: PHONE_TARGET_POS.z,
      duration: 1.0, ease: 'power2.inOut'
    }, 3.0)
    .to(phoneCenter.rotation, {
      x: PHONE_TARGET_ROT.x, y: PHONE_TARGET_ROT.y,
      duration: 1.0, ease: 'power2.inOut'
    }, 3.0)
    ...
    .to(phoneCenter.rotation, { y: '+=' + Math.PI * 1.15, duration: 2, ease: 'sine.inOut' }, 6.0)
    .to(phoneCenter.position, { x: 2.1, y: -0.1, z: -0.5, duration: 2, ease: 'sine.inOut' }, 6.0)
    ...
    // frente do canvas).
    .to(phoneCenter.position, { x: -0.55, y: -0.08, z: -0.1, duration: 1.4, ease: 'power3.inOut' }, 8.1)
    .to(phoneCenter.rotation, { x: 0.02, y: 0.22, z: -0.015, duration: 1.4, ease: 'power3.inOut' }, 8.1)
    ...
    .to(phoneCenter.position, { x: -1.6, y: -0.04, z: 0.4, duration: 1.9, ease: 'power3.out' }, 19.7)
    .to(phoneCenter.rotation, { y: 0.26, x: 0.015, duration: 1.9, ease: 'power3.out' }, 19.7)
    .to(phoneCenter.scale, { x: 0.9, y: 0.9, z: 0.9, duration: 1.9, ease: 'power3.out' }, 19.7)
    .to(opacityCenter, {
