...
(() => {
  ...
  const bgVertexShader = `
    ...
    void main() {
      ...
      // tecido está sempre "respirando", com ou sem o cursor.
      float wave = sin(pos.x * 0.5 + uTime * 0.45) * 0.24
                 + cos(pos.y * 0.42 - uTime * 0.36) * 0.19
                 + sin((pos.x + pos.y) * 0.3 + uTime * 0.58) * 0.12;

      // Deformação real do TECIDO sob o cursor: relevo suave e largo
      // (smoothstep em vez de exponencial estreita) — com a densidade de
      // malha atual (78 segmentos) isso renderiza como uma dobra macia de
      // linho sendo pressionado, não como um facetado geométrico.
      float dist = distance(pos.xy, uMouse);
      float bump = smoothstep(3.1, 0.0, dist) * uMouseActive;
      float bumpSoft = smoothstep(4.4, 0.0, dist) * uMouseActive;

      // O pano se deforma de verdade: ergue-se suavemente sob o cursor
      // (halo mais raso ao redor) — reforça a leitura de tecido físico
      // reagindo ao ponteiro, nunca um buraco/cratera.
      pos.z += wave + bump * 0.85 + bumpSoft * 0.22;
      vBump = bump;
      vWave = wave;
  ...
  const bgFragmentShader = `
    ...
    void main() {
      ...
      // Respiração ambiente: variação de tom entre tinta e os dois
      // acentos aprovados — visível o suficiente para se ler como um
      // pano de linho vivo, mas ainda discreta atrás do texto.
      float breathe = clamp((vWave + 0.5) / 1.0, 0.0, 1.0);
      vec3 clothColor = mix(uInkColor, uGoldColor, breathe * 0.28 + sheenSoft * 0.2 + sheen2 * 0.09);
      vec3 color = mix(clothColor, mix(uGoldColor, uGoldLight, bumpStrength), bumpStrength);

      // Sheen das dobras + respiração ambiente — o "tecido" fica
      // perceptível mesmo longe do cursor, sempre em leve movimento.
      float baseAlpha = sheenSoft * 0.13 + sheen2 * 0.06;
      float breatheAlpha = breathe * 0.06;
      // O dourado segue o cursor com um brilho mais generoso — a área
      // deformada "acende" visivelmente conforme o pano se move com o
      // mouse, sem nunca virar uma mancha sólida opaca.
      float goldAlpha = bumpStrength * 0.62;

      gl_FragColor = vec4(color, clamp(baseAlpha + breatheAlpha + goldAlpha, 0.0, 0.62));
    }
  `;
  ...
  (function setupTelasShowcase() {
    ...
    // 'right' (descanso, recuados atrás do foco).
    ...
    // Deslocamentos calculados a partir da largura real do próprio
    // celular renderizado — assim a composição se adapta sozinha a
    // qualquer breakpoint, sem precisar de um segundo conjunto de
    // números fixos para tablet/desktop.
    function roleTransform(role) {
      const w = (phones[0] && phones[0].offsetWidth) || 240;
      switch (role) {
        case 'active':
          return { x: 0, y: 0, scale: 1, opacity: 1, rotation: 0, zIndex: 3 };
        case 'left':
          return { x: -w * 0.86, y: w * 0.16, scale: 0.72, opacity: 0.5, rotation: -7, zIndex: 1 };
        case 'right':
        default:
          return { x: w * 0.88, y: w * 0.1, scale: 0.72, opacity: 0.5, rotation: 7, zIndex: 1 };
      }
    }

    // Transformação de cada papel quando a formação está "fora do
    // palco": mesma composição horizontal (x), mas empurrada para baixo
    // e invisível — é de lá que ela "sobe" ao chegar, e é para lá que
    // ela "desce" ao sair, continuando o sentido natural do scroll.
    function offstageTransform(role) {
      const t = roleTransform(role);
      return Object.assign({}, t, { y: t.y + 210, opacity: 0 });
    }

    /* -----------------------------------------------------------------
       O 'REVEZAMENTO DE ESTADOS' — equivalente do updateCellState()
       pedido no briefing, só que operando sobre os 3 <div class="telas-
       phone"> reais (DOM/CSS) em vez de meshes 3D. Anima position (x/y),
       rotation, scale e opacity dos TRÊS aparelhos SIMULTANEAMENTE via
       gsap.to(), 0.8s Power3.easeInOut.
       Agora recebe também `onstage`: quando true, a formação assume a
       posição do mockup (chegada/permanência); quando false, os 3
       celulares recuam para fora do palco (antes de chegar / depois de
       sair), sempre respeitando o papel (ativo/esquerda/direita) que
       cada tela ocupa no momento.
       ----------------------------------------------------------------- */
    function setTelasFormation(newScreen, opts) {
      ...
      phones.forEach((el) => {
        ...
        // "Entrada de foco": um celular sobe do eixo Y inferior até o
        // centro sempre que (a) a formação inteira está chegando ao
        // palco, ou (b) ele especificamente está assumindo o papel
        // ativo agora (troca de vitrine/carrinho/conta).
        const risingIntoFocus = role === 'active' && (screenChanged || (onstage && !wasOnstage));
        ...
        if (risingIntoFocus && onstage) {
          gsap.fromTo(
            el,
            { y: target.y + 90, opacity: 0.001 },
            { x: target.x, y: target.y, scale: target.scale, opacity: target.opacity,
              rotation: target.rotation, zIndex: target.zIndex,
              duration: 0.8, ease: 'power3.inOut', overwrite: 'auto' }
          );
        } else {
          gsap.to(el, {
            x: target.x, y: target.y, scale: target.scale, opacity: target.opacity,
    ...
    // para "subir" assim que a seção entrar na tela.
    ...
    /* -----------------------------------------------------------------
       ...
       - TOPO: ao se aproximar da seção (descendo), os 3 celulares
         SOBEM para a posição do mockup; ao voltar para cima (subindo,
         saindo da seção por cima), eles recuam de novo para fora do
         palco.
       - BASE: ao continuar descendo (saindo da seção por baixo), os 3
         celulares DESCEM/some para fora do palco, dando lugar à
         próxima seção; ao voltar (subindo de novo para dentro da
         seção), eles retornam à formação.
       Nenhum dos dois é "scrub contínuo" — são transições de 0.8s que
       disparam uma única vez por direção, como o resto da coreografia
       de estados. ----------------------------------------------------- */
    ScrollTrigger.create({
      trigger: section,
      start: 'top 82%',
